...
import AddCategoriesThree from "./AddCategoriesThree";
import Pagination from "../../Pagination";
import { domain } from "../../../../security";

// Mobile Card Component
const CategoryThreeCard = ({ category, onEdit, onDelete }) => (
...
const AllCategoriesThree = () => {
  const [categoryThreeData, setCategoryThreeData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [CategoryToEdit, setCategoryToEdit] = useState(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [categoriesThreePerPage] = useState(10);
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredCategoriesThree, setFilteredCategoriesThree] = useState([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setCategoryThreeData(response.data);
      setLoading(false);
    } catch (error) {
      console.error("Error fetching data:", error);
      toast.error("Failed to fetch CategoriesThree.");
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    const results = categoryThreeData.filter((categoryThree) =>
      categoryThree.categoryThreeName
        .toLowerCase()
        .includes(searchTerm.toLowerCase())
    );
    setFilteredCategoriesThree(results);
    setCurrentPage(1);
  }, [searchTerm, categoryThreeData]);

  const deleteCategoryThree = async (id) => {
    const apiUrl = `${domain}/api/CategoriesThree/${id}`;
  ...
  const indexOfLastCategoryThree = currentPage * categoriesThreePerPage;
  const indexOfFirstCategoryThree =
    indexOfLastCategoryThree - categoriesThreePerPage;
  const currentCategoriesThree = filteredCategoriesThree.slice(
    indexOfFirstCategoryThree,
    indexOfLastCategoryThree
  );
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                <Layers className="text-indigo-600" /> Category Level 3
              </h1>
              <p className="text-sm text-slate-500 mt-1">
                Manage third-level product categorization.
              </p>
            </div>
            <button
              onClick={() => openModal()}
      ...
      <div className="max-w-[95%] mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        ...
        {loading ? (
          ...
          <>
            <div className="hidden md:block bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              <table className="w-full text-sm text-left text-slate-600">
                ...
                <tbody className="divide-y divide-slate-100">
                  {currentCategoriesThree.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={() => deleteCategoryThree(item.id)}
                  ...
                  {currentCategoriesThree.length === 0 && (
                    <tr>
                      <td
                        colSpan="2"
            ...
            <div className="mt-6">
              <Pagination
                itemsPerPage={categoriesThreePerPage}
                totalItems={filteredCategoriesThree.length}
...
export default AllCategoriesThree;
