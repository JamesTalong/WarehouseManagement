...
import AddCategoriesTwo from "../CategoryModule/AddCategoriesTwo";
import Pagination from "../../Pagination";
import { domain } from "../../../../security";

// Mobile Card Component
const CategoryTwoCard = ({ category, onEdit, onDelete }) => (
...
const AllCategoriesTwo = () => {
  ...
  const [categoriesTwoPerPage] = useState(10);
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredCategoriesTwo, setFilteredCategoriesTwo] = useState([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to fetch CategoriesTwo.");
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    const results = categoryTwoData.filter((categoryTwo) =>
      categoryTwo.categoryTwoName
        .toLowerCase()
        .includes(searchTerm.toLowerCase())
    );
    setFilteredCategoriesTwo(results);
    setCurrentPage(1);
  }, [searchTerm, categoryTwoData]);

  const deleteCategoryTwo = async (id) => {
    const apiUrl = `${domain}/api/CategoriesTwo/${id}`;
  ...
  const openModal = (categoryTwo = null) => {
    setCategoryToEdit(categoryTwo);
  ...
  const indexOfLastCategoryTwo = currentPage * categoriesTwoPerPage;
  const indexOfFirstCategoryTwo = indexOfLastCategoryTwo - categoriesTwoPerPage;
  const currentCategoriesTwo = filteredCategoriesTwo.slice(
    indexOfFirstCategoryTwo,
    indexOfLastCategoryTwo
  );
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                <Layers className="text-indigo-600" /> Category Level 2
              </h1>
              <p className="text-sm text-slate-500 mt-1">
                Manage second-level product categorization.
              </p>
            </div>
            <button
              onClick={() => openModal()}
      ...
      <div className="max-w-[95%] mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        ...
        {loading ? (
          ...
          <>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={() => deleteCategoryTwo(item.id)}
            ...
            <div className="mt-6">
              <Pagination
                itemsPerPage={categoriesTwoPerPage}
                totalItems={filteredCategoriesTwo.length}
